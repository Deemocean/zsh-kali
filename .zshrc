...
configure_prompt() {
    ...
    case "$PROMPT_ALTERNATIVE" in
        twoline)
            PROMPT=$'%F{%(#.blue.green)}┌──${debian_chroot:+($debian_chroot)─}${VIRTUAL_ENV:+($(basename $VIRTUAL_ENV))─}(%B%F{%(#.red.blue)}%n'$prompt_symbol$' %m%b%F{%(#.blue.green)})-[%B%F{reset}%(6~.%-1~/…/%4~.%5~)%b%F{%(#.blue.green)}]%F{011}${vcs_info_msg_0_}\n%F{green}└─%B%(#.%F{red}#.%F{blue}$)%b%F{reset} '
...
#source ~/workspace/traxxas_ws/devel/setup.zsh
alias code='code --disable-accelerated-video-decode'
